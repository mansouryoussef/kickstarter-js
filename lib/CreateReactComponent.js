...
class CreateReactComponent {
  constructor(component_name, typescript, scss, stylesheet_module) {
    ...
    this.component_extension = typescript ? ".ts" : ".js";
    this.scss = scss;
  }
...
export default function test({}: Props): ReactElement {
  return (
    ...
    fs.writeFileSync(
      `${this.component_name}/${this.component_name}.ts`,
      template,
      (err) => {
        if (err) {
          return console.log(err);
  ...
  create_component_css = () => {
    fs.writeFileSync(
      `${this.component_name}/${this.component_name}Styles.css`,
      "",
      (err) => {
        if (err) {
          return console.log(err);
    ...
    const css_import = `import '${this.component_name}Styles.css'`;
  ...
  create_component_scss = () => {
    fs.writeFileSync(
      `${this.component_name}/${this.component_name}Styles.scss`,
      "",
      (err) => {
        if (err) {
          return console.log(err);
    ...
    const scss_import = `import '${this.component_name}Styles.scss'`;
  ...
  create_component_css_module = () => {
    fs.writeFileSync(
      `${this.component_name}/${this.component_name}Styles.module.css`,
      "",
      (err) => {
        if (err) {
          return console.log(err);
    ...
    const css_module_import = `import Styles from '${this.component_name}Styles.module.css'`;
  ...
  create_component_scss_module = () => {
    fs.writeFileSync(
      `${this.component_name}/${this.component_name}Styles.module.scss`,
      "",
      (err) => {
        if (err) {
          return console.log(err);
    ...
    const scss_module_import = `import Styles from '${this.component_name}Styles.module.scss'`;
