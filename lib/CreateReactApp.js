...
const { isRequired, is_lowercase } = require("../utils/validation");
require("colors");
...
class CreateReactApp {
  ...
  check_for_project_name = async () => {
    if (!this.project_name || !is_lowercase(this.project_name)) {
      if (!is_lowercase(this.project_name)) {
        console.log(
          "Error: Create React App does not support uppercase letters.".red
        );
      }

      let user_input = await this.get_project_name_from_user();

      while (is_lowercase(user_input) === false) {
        console.log(
          "Error: Create React App does not support uppercase letters.".red
        );

        user_input = await this.get_project_name_from_user();
      }
